    ...
    this.subscriptions.add(this.onDidMessageAdd(message => {
      ...
      if (this.gutterEnabled) {
        ...
        item.addEventListener('click', (e) => {
          ...
          this.isShouldShowBubble = true;

          let theMessage = this.findFirstMessageInRow(message.range.start.row);
          if (theMessage) {
            // theMessage = message;
            const range = theMessage.range;
            const bubbleMarker = this.editor.markBufferRange(range, {invalidate: 'inside'});

            this.bubble = {bubbleMarker};
            this.editor.decorateMarker(bubbleMarker, {
              type: 'overlay',
              item: createBubble(theMessage)
            })

            atom.workspace.getActiveTextEditor().setCursorBufferPosition(theMessage.range.start);
          }
        });
